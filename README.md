...
Invoicer runs entirely in your browser. Create professional invoices, generate PDFs, manage clients and brands, and track follow-ups — all stored in `localStorage`. Your data never leaves your machine.
...
There is no Settings screen and no login/auth screen — both are omitted by design; see below.
...
- **Auth.** There is no login, no session, no sign-out. The app is single-user by construction — whoever opens the browser is "you."
...
None of this is a bug — it's what a client-only, no-backend build of this handoff can honestly support. Treat the Pro tier and every follow-up "sent" timestamp as illustrative, not real.
...
|--------------------|----------|
| `invoicer_brands` | Brands, including accent colour and follow-up config |
| `invoicer_clients` | Saved clients |
| `invoicer_invoices` | All invoices, including their frozen brand snapshot and reminder history |
| `invoicer_templates` | Reusable follow-up email templates |
| `invoicer_plan` | The (mock) Pro/Free flag |
| `invoicer_schema_version` | Tracks whether the v1→v2 migration has run |
...
The first time you open the app after updating, a migration runs automatically and once, in place:
...
The migration is idempotent and safe to interrupt (e.g. a full `localStorage` quota mid-write) — it picks up wherever it left off on the next load, and never marks itself complete over a partial write.
